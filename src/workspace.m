bool workspace_event_handler_begin(void **context) {
  ...
  if (version.majorVersion >= 27)
    version.majorVersion = 26; // alias macOS 27+ to Tahoe(26): treat newer
                               // betas as Tahoe until real offsets land
...
void *
workspace_application_create_running_ns_application(struct process *process) {
  return [[NSRunningApplication
      runningApplicationWithProcessIdentifier:process->pid] retain];
}

void workspace_application_destroy_running_ns_application(
    void *ws_context, struct process *process) {
  NSRunningApplication *application =
      __atomic_load_n(&process->ns_application, __ATOMIC_RELAXED);
  ...
  if (application) {
    if ([application observationInfo]) {
      ...
      // :WorstApiEverMade
      //
      // NOTE(asmvik): Because the developers of this API did such an amazing
      // job there is no way for us to actually just friggin loop through the
      // currently registered observations and then call removeObservation on
      // them..
      //
      // Instead we just try to force remove the observations that **could** be
      // present at this point in time, because it will complain if we try to
      // actually release the object when it has observers present.
      //
      // We can't actually correctly track whether it did actually get
      // unobserved previously, because even when our notification callback is
      // triggered it will claim that we try to remove a non-existing
      // observation when it just called us back.
      //

      @try {
        [application removeObserver:ws_context
...
void workspace_application_observe_finished_launching(void *context,
...
void workspace_application_observe_activation_policy(void *context,
                                                     struct process *process) {
  NSRunningApplication *application =
      __atomic_load_n(&process->ns_application, __ATOMIC_RELAXED);
  if (application) {
...
void workspace_application_unobserve(void *ws_context,
                                     struct process *process) {
  NSRunningApplication *application =
      __atomic_load_n(&process->ns_application, __ATOMIC_RELAXED);
  if (application) {
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:@"activationPolicy"]) {
    ...
    if ([result intValue] != process->policy) {
      ...
      // :WorstApiEverMade
      //
      // NOTE(asmvik): For some stupid reason it is possible to get notified by
      // the system about a change, and NOT being able to remove ourselves from
      // observation because it claims that we are not observing the key-path,
      // but we clearly are, as we would otherwise not be here in the first
      // place..
      //
...
- (void)didHideApplication:(NSNotification *)notification {
  pid_t pid = [[notification.userInfo objectForKey:NSWorkspaceApplicationKey]
      processIdentifier];
